...
import type { UserProfile, Scenario, InterviewSession } from "@/types";
...
// sessions
export async function saveSession(session: Omit<InterviewSession, "id">): Promise<string> {
    ...
    await updateDoc(doc(db, "users", session.userId), {
        sessionsCompleted: (await getUserProfile(session.userId))!.sessionsCompleted + 1,
    });
    return ref.id;
}
